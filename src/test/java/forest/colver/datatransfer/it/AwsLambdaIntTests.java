...
import static forest.colver.datatransfer.aws.Utils.getS3SourceCache;
...
import static forest.colver.datatransfer.aws.Utils.getSqs1;
...
public class AwsLambdaIntTests {
  ...
  private static final String SANDBOX_SFTP_SOURCE_CAHCE_BUCKET = getS3SourceCache();
  ...
  public void testInvokeLambda() {
    ...
    var queue = getSqs1();
    var messageResp = sqsGet(creds, queue);
    ...
    messageResp = sqsGet(creds, queue);
    assertThat(messageResp.hasMessages()).isTrue();
    assertThat(messageResp.messages().get(0).body()).contains(
        "\"key\": \"ext-aiko1/outbound/dev/flox/dd/1test.txt\"");

    // Cleanup the queue and the s3.
    sqsDelete(creds, messageResp, queue);
    s3Delete(creds, SANDBOX_SFTP_SOURCE_CAHCE_BUCKET, objectKey);
  }
