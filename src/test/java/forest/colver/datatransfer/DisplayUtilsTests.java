...
import static forest.colver.datatransfer.messaging.DisplayUtils.displayMessage;
...
public class DisplayUtilsTests {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(DisplayUtilsTests.class);

  @Test
  public void testCreateStringTextMessage() {
    var message = createMessage();
    var s = stringFromMessage(message);
  ...
  public void testCreateStringSimpleTextMessage() {
    ...
    assertThat(stringFromMessage(message, 10, false))
        .isEqualTo(
            "\n"
                + "Message Type: Text\n"
                + "  Custom Properties:\n"
  ...
  public void testCreateStringTruncatePayload() {
    ...
    var s = stringFromMessage(message, 16, false);
    LOG.info(s);
    assertThat(s).contains("Message Type: Text");
  ...
  public void testCreateStringWithJmsProps() {
    var message = createMessage();
    var s = stringFromMessage(message, 100, true);
    LOG.info(s);
    assertThat(s).contains("Message Type: Text");
    assertThat(s).contains("Payload (truncated to 100 chars): Default Payload:");
