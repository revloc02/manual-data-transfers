...
public class CloudWatchLogsOps {
  ...
  public static PutLogEventsResponse putCWLogEvents(
      AwsCredentialsProvider awsCp, String logGroupName, String streamName, List<String> messages) {
  ...
  public static PutLogEventsResponse putCWLogEvents(
    ...
    try (var logsClient = getCloudWatchLogsClient(awsCp)) {
      ...
      for (String s : messages) {
        ...
        var inputLogEvent =
            InputLogEvent.builder().message(s).timestamp(System.currentTimeMillis()).build();
  ...
  private static String getSequenceToken(
      CloudWatchLogsClient logsClient, String logGroupName, String streamName) {
    ...
    // Assume that a single stream is returned since a specific stream name was specified in the
    // previous request.
